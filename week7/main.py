...
app = FastAPI()
app.add_middleware(SessionMiddleware, secret_key="secret") # 設定 session 的密鑰
...
async def query_member(request: Request, username: str = Query(...)):
    if request.session.get("SIGNED-IN"):
        try:
            ...
            print("Error while querying member:", e)
            return {"data": None}
    else:
        return RedirectResponse(url="/", status_code=303)
...
# Endpoint for name updating
@app.patch("/api/member", response_class=JSONResponse)
async def update_name(request: Request, updateModel: updateModel = Body(...)):
    if request.session.get("SIGNED-IN"):
        try:
            cursor.execute("UPDATE `member` SET name = %s WHERE id = %s", (updateModel.name, request.session["member_id"]))
            con.commit()
            request.session["member_name"] = updateModel.name
            return {"ok": True}
        except Exception as e:
            print("更新名稱時發生錯誤:", e)
            return {"error": True}
